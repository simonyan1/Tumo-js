...
function drawRectangle(width, height) {
    ...
    ctx.rect(ellipseCenterX, ellipseCenterY - height / 2, width, height, 0, 0, 2 * Math.PI);
    ctx.stroke();
...
function color(...clr) {
    ...
    else if (typeof clr[0] == "string" && clr[0].startsWith("#") && clr[0].length == 7) {
...
function drawStar(points, size) {
    ...
    const angleSteps = Math.PI / points;
...
function width(w) {
	if (typeof w !== "number" || w < 0) {
		console.error("Invalid width. Width must be a non-negative numerical value.");
		return ;
	}
	ctx.lineWidth = w;
}
...
function fillColor(...clr) {
    ...
    if(Array.isArray(...clr) && clr.length == 1)
        ...
        cl = clr
    if (cl != undefined && typeof cl[0] === "number" && cl[0]<=255 && cl[0]>=0 &&
        typeof cl[1] === "number" && cl[1]<=255 && cl[1]>=0 &&
        typeof cl[2] === "number" && cl[2]<=255 && cl[2]>=0 ) {
...
function move(length) {
    if (typeof length !== "number" || length < 0) {
        console.error("Invalid length. Length must be a non-negative numerical value.");
        return;
    }
    const radianAngle = (currentAngle * Math.PI) / 180;
    ...
    if (newX < 0 || newX > canvas.width || newY < 0 || newY > canvas.height) {
        console.error("Out of canvas, increase the canvas size, or change the length.");
        return;
    }
    currentX = newX;
    currentY = newY;
